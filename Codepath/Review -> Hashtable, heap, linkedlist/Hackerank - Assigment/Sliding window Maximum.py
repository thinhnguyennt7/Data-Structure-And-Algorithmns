"""
Given an array nums, there is a sliding window of size k which is moving from the very left to the very right. you can only see the k numbers in the windows
each time the sliding window moves right by one positiion. Return the max sliding window.
...
[1  3  -1] -3  5  3  6  7       3
 ...
 Note: You may assum k is always. valid, 1 <= k <= inout array's zire for non-empty array.
